...
function clean_up_vpc() {
    if [[ $(gcloud compute networks list \
        ...
        for subnet in $(gcloud compute networks subnets list --filter="NETWORK:${VPC_NAME} AND ROLE:BACKUP" --format="csv(NAME,REGION)[no-heading]")
        do
...
gcloud compute networks subnets list --format="table(NAME,REGION,NETWORK,RANGE,PURPOSE,ROLE)" --filter="NETWORK:${VPC_NAME}"

log INFO "re-use existing modules but add support for proxy-only subnets"
...
log INFO "as we can see error above, re-use existing gcp_compute_subnetwork module"
log INFO "add properties to existing module and input variables will break existing infra"
log INFO "for existing infra defined in the subnets.json, use-case team will have to add new properties"
log INFO "we will try add another module for proxy-only subnet"

log INFO "prepare main.tf, add new module for proxy-only subnet"
cat >${SCRIPT_DIR}/main.tf<<EOF
locals {
  subnets = {
    for subnet, value in var.subnets :
...
log INFO "prepare vars.tf, add a new input variable for proxy-only subnets"
cat >${SCRIPT_DIR}/vars.tf<<EOF
variable "project" {
  description = "project ID"
  type        = string
